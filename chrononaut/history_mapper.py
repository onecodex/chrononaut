...
def history_mapper(local_mapper):
    ...
    if not super_mapper or local_mapper.local_table is not super_mapper.local_table:
        ...
        history_tablename = getattr(
            cls, "__chrononaut_tablename__", local_mapper.local_table.name + "_history"
        )
        table = Table(
            history_tablename,
    ...
    for col_name in untracked_cols:
        ...
        setattr(versioned_cls, col_name, property(lambda _: raise_(UntrackedAttributeError(msg))))

    for col_name in hidden_cols:
        msg = "{} is explicitly hidden via __chrononaut_hidden__".format(col_name)
        setattr(versioned_cls, col_name, property(lambda _: raise_(HiddenAttributeError(msg))))
